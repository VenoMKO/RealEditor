  ...
  : wxDialog(parent, wxID_ANY, wxT("Export options"), wxDefaultPosition, wxSize(597, 457), wxDEFAULT_DIALOG_STYLE)
{
	...
	StaticMeshes->SetToolTip(wxT("Export Static Mesh actors"));
	...
	SkeletalMeshes->SetToolTip(wxT("Export Skeletal Mesh actors"));
	...
	SpeedTrees->SetToolTip(wxT("Export SpeedTree actors"));
	...
	Interps->SetToolTip(wxT("Export Interp actors. Converts Interp actors to Static Mesh actors"));
	...
	Terrains->SetToolTip(wxT("Export Terrain actors"));
	...
	PointLights->SetToolTip(wxT("Export Point Light actors"));
	...
	SpotLights->SetToolTip(wxT("Export Spot Light actors"));
	...
	SoundNodes->SetToolTip(wxT("Export Sound actors"));
	...
	ResampleTerrain->SetToolTip(wxT("Resize terrain heightmap to match weightmap resolution. Allows to use original full resolution weightmaps, but changes original terrain geometry"));
	...
	SplitTerrainWeightMaps->SetToolTip(wxT("Save weightmap channels in individual files"));
	...
	m_notebook1->AddPage(m_panel15, wxT("Textures"), false);
