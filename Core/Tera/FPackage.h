...
struct FPackageDumpHelper {
  struct CompositeDumpEntry {
    struct CompositeDumpExport {
    ...
    FString ObjectPath;
    std::vector<CompositeDumpExport> Exports;
  };

  // Returns all composite GPK files
  static std::vector<FString> GetGpkPools();
  // Returns path to a composite GPK file
...
public:
  ...
  static FString GetDcPath(const FString& s1data = {});
